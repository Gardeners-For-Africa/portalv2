...
export default function Login() {
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center p-4 sm:p-6 md:p-8">
      {/* Background Image */}
      <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1540151812223-c30b3fab58e6')] bg-cover bg-center" />

      {/* Overlay */}
      <div className="absolute inset-0 bg-green-900/70" />

      <div className="w-full relative max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
        {/* LEFT SIDE */}
        <div className="text-center lg:text-left text-primary-foreground space-y-6 px-2 sm:px-4 lg:px-0">
          <div className="flex items-center justify-center lg:justify-start gap-3">
            <div className="w-10 h-10 sm:w-12 sm:h-12 bg-primary-foreground/20 rounded-lg flex items-center justify-center">
              <PortalLogo size={28} className="sm:size-8 text-primary-foreground" />
            </div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-poppins font-bold">
              Gardeners for Africa
            </h1>
          </div>

          <div className="space-y-4">
            <h2 className="text-xl sm:text-2xl font-poppins font-semibold max-w-md md:mx-auto">
              Welcome to the Future of Education Management
            </h2>
            <p className="text-base sm:text-lg text-primary-foreground/80 max-w-md md:mx-auto">
          ...
          <div className="grid grid-cols-2 gap-4 max-w-md md:mx-auto">
            <div className="bg-primary-foreground/10 rounded-lg p-3 sm:p-4 text-center">
              <div className="text-xl sm:text-2xl font-bold">15K+</div>
              <div className="text-xs sm:text-sm text-primary-foreground/70">Students</div>
            </div>
            <div className="bg-primary-foreground/10 rounded-lg p-3 sm:p-4 text-center">
              <div className="text-xl sm:text-2xl font-bold">900+</div>
              <div className="text-xs sm:text-sm text-primary-foreground/70">Teachers</div>
            </div>
          </div>
        </div>
        ...
        <div className="w-full max-w-md mx-auto space-y-6">
          <Card className="shadow-strong border-0 bg-card/95 backdrop-blur">
            <CardHeader className="text-center">
              <CardTitle className="text-xl sm:text-2xl font-poppins">Sign In</CardTitle>
            ...
            <Card className="shadow-medium border-0 bg-card/95 backdrop-blur">
              ...
              <CardContent className="space-y-2">
                {DEMO_ACCOUNTS.map((account) => (
                  <div
                    key={account.email}
                    className="flex md:items-center justify-between p-3 rounded-lg bg-muted/50 hover:bg-muted cursor-pointer transition-colors"
                    onClick={() => handleDemoLogin(account.email, account.password)}
                  >
                    <div className="flex flex-col md:mb-2 mb-0">
                      <span className="text-xs sm:text-sm font-medium">{account.email}</span>
